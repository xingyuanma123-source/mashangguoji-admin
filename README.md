...
先安装 Node.js 20（20.12 或更高）和 pnpm 10.30.3。在仓库根目录，只需记住四个命令：

| 操作 | 命令 |
| --- | --- |
| 第一次安装依赖 | `npm run setup` |
| 启动客服后台 | `npm run dev` |
| 启动司机小程序 | `npm run dev:miniapp` |
| 提交前检查 | `npm run check` |

后台首次启动需要负责人配置测试库的 `webpage/db-proxy/.env`。缺配置时会有中文提示；启动脚本自动拉起本地数据库代理和前端。AI/OCR 按需配置。以上入口在当前汇总分支可用，合入主线需完成相关 PR 审核。
