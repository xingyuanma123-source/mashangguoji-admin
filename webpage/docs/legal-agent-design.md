...
- **模型**：合同抽取、风险扫描和 Agent 主循环统一使用 DeepSeek V4 Pro；浏览器不持有模型密钥。
...
1. **模型 function calling 稳定性**：DeepSeek V4 Pro 通过 OpenAI 兼容接口承接工具调用、多轮回喂和流式输出，需持续监控上游稳定性。
